...
import { CoachAuthProvider, useCoachAuth } from "@/components/coach/auth-provider";
import { CoachSidebar, MobileMenuButton } from "@/components/coach/sidebar";
import { ToastContainer } from "@/components/ui/toast";
import { Loader2, LogOut } from "lucide-react";

function CoachLayoutContent({ children }: { children: React.ReactNode }) {
  const { user, loading, logout } = useCoachAuth();
  ...
  // Redirect handled by CoachAuthProvider if not logged in
  if (!user) {
    return (
      <div className="flex min-h-screen items-center justify-center bg-neutral-50">
        <Loader2 className="h-8 w-8 animate-spin text-neutral-400" />
