...
import { AttendanceSummary, SessionType } from "@/types/attendance";
      ...
      {loading ? (
        ...
        <>
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
              <div className="bg-white rounded-2xl border border-neutral-200/60 p-4 shadow-[0_1px_3px_rgba(0,0,0,0.04)]">
                <p className="text-[13px] text-neutral-500 mb-1">
                  Sessions{hasActiveFilters ? " (Filtered)" : " Today"}
                </p>
          ...
          {!summary || summary.sessions.length === 0 ? (
            <AdminEmptyState
              icon={ClipboardCheck}
              title={hasActiveFilters ? "No matching sessions" : "No sessions scheduled"}
              description={
                hasActiveFilters
                  ? "No sessions match your current filters. Try adjusting your filters."
                  : `No sessions are scheduled for ${formatDate(selectedDate)}`
              }
            />
          ) : (
            <ResponsiveTable
