...
import { ArrowLeft, Loader2, Percent, Users, ShoppingBag, Clock } from "lucide-react";
import { CreateDiscountRuleInput } from "@/types/discount-rule";
...
export default function NewDiscountPage() {
  ...
  const [formData, setFormData] = useState<CreateDiscountRuleInput>({
    name: "",
    description: "",
    type: "sibling",
    conditions: {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        toast("Discount rule created successfully", "success");
        router.push("/admin/discounts");
      } else {
        setError(data.error || "Failed to create discount rule");
        toast(data.error || "Failed to create discount rule", "error");
      }
    } catch (err) {
      const errorMessage =
      ...
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-neutral-500 text-sm mt-1">
            Create a new discount rule for your bookings
          </p>
        </div>
      </div>
                ...
                <div className="flex items-center gap-3">
                  <button
                    ...
                    onClick={() =>
                      setFormData({
                        ...formData,
                        discount: { ...formData.discount, appliesTo: "additional" },
                      })
                    }
                    className={`px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
          ...
          <AdminCard hover={false}>
            ...
            <div className="space-y-3 text-sm text-neutral-600">
              {formData.type === "sibling" && (
                <>
                  <p>
                    <strong>Sibling discounts</strong> apply when a family books
                    sessions for multiple children.
                  </p>
                  <p>
                    The discount is calculated based on unique child names in the
                    cart.
                  </p>
                </>
              )}
              {formData.type === "bulk" && (
                <>
                  <p>
                    <strong>Bulk discounts</strong> apply when a customer books
                    multiple sessions in a single order.
                  </p>
                  <p>Great for encouraging larger bookings.</p>
                </>
              )}
              {formData.type === "early_bird" && (
                <>
                  <p>
                    <strong>Early bird discounts</strong> reward customers who
                    book in advance.
                  </p>
                  <p>
                    The discount applies to sessions starting at least the
                    specified days from now.
                  </p>
                </>
              )}
            </div>
